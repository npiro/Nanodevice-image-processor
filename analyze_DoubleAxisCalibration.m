...
foldername =  '\\lpqm1srv3.epfl.ch\Nano\DAQsoftware\Image processing\20150710_C705_HalfHalf_SDSBridge_Pad_2mm';
figurename='C705_155_1C_25';
...
Nh = length(holes);
d = zeros(Nh-1,1);
beamrotation=atan(abs(elip{end}.Y0_in-elip{1}.Y0_in)/abs(elip{end}.X0_in-elip{1}.X0_in));
...
for i = 1:Nh-1
    d_X(i) = abs(elip{i+1}.X0_in-elip{i}.X0_in)/cos(beamrotation);
end
figure('name','Consecutive holes X spacing distance');plot(d_X,'xb');xlabel('Hole Number');ylabel('Distance X-Axis between successive holes [pix]');


for i = 1:Nh-1
    d_Y(i) = abs(elip{i+1}.Y0_in-elip{i}.Y0_in)/sin(beamrotation);
end
figure('name','Consecutive holes Y spacing distance');plot(d_Y,'xb');xlabel('Hole Number');ylabel('Distance Y-Axis between successive holes [pix]');

rotation=atan(abs(elip{end}.Y0_in-elip{1}.Y0_in)/abs(elip{end}.X0_in-elip{1}.X0_in));
...
MeanScalingFactorPitch=mean(ScalingFactorPitch)
...
%Calibration with X axis.
for i=1:(length(holes)-1)
ScalingFactorPitch_X(i)=d_X(i)/Design_Spacing((abs(i-Ncenter+0.5))+0.5);
end
MeanScalingFactorPitch_X=mean(ScalingFactorPitch_X)
figure('name','Deviation of the Pitches with X axis calibration')
plot(linspace(1,(length(holes)-1),length(holes)-1)-Ncenter,ScalingFactorPitch_X/mean(ScalingFactorPitch_X)-1,'o');
xlabel('Hole Number');
ylabel('Deviation of the Pitches');

%Calibration with Y axis.
for i=1:(length(holes)-1)
ScalingFactorPitch_Y(i)=d_Y(i)/Design_Spacing((abs(i-Ncenter+0.5))+0.5);
end
figure('name','Deviation of the Pitches with Y axis calibration ')
MeanScalingFactorPitch_Y=mean(ScalingFactorPitch_Y)
plot(linspace(1,(length(holes)-1),length(holes)-1)-Ncenter,ScalingFactorPitch_Y/mean(ScalingFactorPitch_Y)-1,'o');
xlabel('Hole Number');
ylabel('Deviation of the Pitches');
