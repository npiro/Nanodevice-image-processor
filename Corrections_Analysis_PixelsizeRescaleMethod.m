...
%load('Z:\DAQsoftware\Image processing\C0625_155_2D\imData.mat')
%%
nSEM = length(imData);
    ...
    for j=1:nEllipses(i)
        ...
        for k=1:(HoleMax-HoleMin+1)
            if EllipsesIndex(k)>=min(HoleIndex) & EllipsesIndex(k)<max(HoleIndex)
                ...
                EllipsesRadius_X(k,i)=Radius_X(find(HoleIndex==EllipsesIndex(k)))/fitresult{i}(v(HoleIndex==EllipsesIndex(k)));
...
%% Plots of the scaling Factor of the pitch       
        figure(4)
        validPitch=1:(length(EllipsesIndex)-1);
        PitchIndex=EllipsesIndex(validPitch);
        scatter(PitchIndex,EllipsePitchScaling,'s')
xlabel('Ellipse Index')
ylabel('Pitch Scaling Factor')
% Fitting of the Pitch Scaling Factor
[xData, yData] = prepareCurveData( PitchIndex, EllipsePitchScaling );
% Set up fittype and options.
ft = fittype( 'poly4' );
% Fit model to data.
...
%% reScaled Hx
for kk=1:(HoleMax-HoleMin+1)
    ScalingFactor(kk)=fitresult(EllipsesIndex(kk));
    MeanEllipsesRadius_X_SEMScaled(kk)=mean(EllipsesRadius_X(kk,(EllipsesRadius_X(kk,:)~=0)))/ScalingFactor(kk);
    MeanEllipsesDifference_X_SEMScaled(kk)=MeanEllipsesRadius_X_SEMScaled(kk)-HxVec(abs(EllipsesIndex(kk))+1);
end
figure(3)
scatter(EllipsesIndex,MeanEllipsesDifference_X_SEMScaled)
xlabel('Ellipses Index')
ylabel('Difference between measured and design for Hx rescaled with Pitch')
%% Fitting of 
...
FittedMeanHx(i)=a + b * exp(-(abs(EllipsesIndex(i))/c)^d )
end
figure(2)
hold on
scatter(EllipsesIndex,MeanEllipsesDifference_X);
